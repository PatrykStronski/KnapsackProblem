...
fn calculate_children_capacity(number: u16) -> usize {
    let res: f32 = (number as f32) / 2.0;
    if res.fract() != 0.0 {
        return (res + 1.0) as usize;
    }
    return res as usize;
}
...
pub fn crossover_all(mut individuals: Vec<Vec<bool>>, crossover_rate: f32) -> Vec<Vec<bool>> {
    ...
    let mut children = Vec::<Vec<bool>>::with_capacity(calculate_children_capacity(number));
    let mut iter: u16 = 0;
    while iter < number-1 {
        let n = iter as usize;
        let mut i1 = individuals[n].to_vec();
        let mut i2 = individuals[n+1].to_vec();
        crossover(i1, i2, crossover_rate);
        iter=iter+2;
    }
    return children;
}
